"""
Update sample products with Unsplash image URLs
This script updates existing products to use free Unsplash images
"""
...
class Command(BaseCommand):
    help = 'Update sample products with Unsplash image URLs'

    def handle(self, *args, **options):
        self.stdout.write('Updating products with image URLs...')
        ...
        for product_name, image_url in product_images.items():
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f'✗ Not found: {product_name}')
                )
        ...
        if not_found:
            self.stdout.write(
                self.style.WARNING(f'✗ {len(not_found)} products not found')
            )
